...
export const revalidate = 60 // revalida cada 60s — datos frescos sin over-fetching
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-text-muted font-mono mt-0.5">
            Seoul Kims · Viña del Mar ·{' '}
            {new Date(stats.generatedAt).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}
          </p>
        </div>
        <RefreshButton />
      </div>

      {/* Alertas críticas — DTE fallido o producto vencido en góndola */}
      ...
      {alerts.vencidos.length > 0 && (
        <AlertBanner
          severity="critical"
          title={`${alerts.vencidos.length} producto${alerts.vencidos.length > 1 ? 's' : ''} VENCIDO${alerts.vencidos.length > 1 ? 'S' : ''} con stock en góndola`}
      ...
      {/* KPIs — 8 métricas reales Seoul Kims */}
      <section>
        <h2 className="font-headline text-sm font-semibold text-text-muted uppercase tracking-wide mb-3">
          Hoy
        </h2>
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
          <KPICard
            ...
            format="clp"
            delta={stats.deltaVentas}
            variant={stats.ventasHoy === 0 ? 'default' : 'success'}
          />
          <KPICard
          ...
          <KPICard
            ...
            format="number"
            variant={stats.pedidosActivos > 10 ? 'warning' : 'default'}
          />
          <KPICard
            label="Web sin despachar"
            value={stats.pedidosWebSinDespachar}
            format="number"
            variant={stats.pedidosWebSinDespachar > 5 ? 'urgent' : 'default'}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
          <KPICard
            ...
            value={stats.vencenEstaSemana}
            format="number"
            variant={stats.vencenEstaSemana > 0 ? 'warning' : 'success'}
          />
          <KPICard
            label="Stock crítico"
            value={stats.stockCritico}
            format="number"
            variant={stats.stockCritico > 0 ? 'warning' : 'success'}
          />
          <KPICard
            label="B2B sin cobrar"
            value={stats.b2bPendientes}
            format="number"
            variant={stats.b2bPendientes > 0 ? 'warning' : 'default'}
          />
          <KPICard
            label="DTE fallidos"
            value={alerts.dtesFallidos.length}
            format="number"
            variant={alerts.dtesFallidos.length > 0 ? 'urgent' : 'success'}
          />
        </div>
      </section>
...
function RefreshButton() {
  return (
    <form action="/cerebro/dashboard">
      <button
        type="submit"
        className="flex items-center gap-1.5 text-xs text-text-muted hover:text-text transition-colors px-3 py-1.5 rounded-md hover:bg-surface"
      >
        <RefreshCw size={12} />
        Actualizar
      </button>
