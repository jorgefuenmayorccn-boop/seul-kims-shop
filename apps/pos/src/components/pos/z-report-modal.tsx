...
export function ZReportModal({ shiftId, apiUrl, onClose, onLogout }: ZReportModalProps) {
  ...
  useEffect(() => {
    // Try fetching live Z-Report (shift still open)
    fetch(`${apiUrl}/api/shifts/${shiftId}/z-report`, { credentials: 'include' })
      .then(r => r.json() as Promise<{ zReport: ZReport }>)
  ...
  const handleClose = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json() as { zReport: ZReport; error?: string }
          ...
          <div>
            ...
            {report && (
              <p className="font-mono text-xs" style={{ color: 'var(--color-text-muted)' }}>
                Turno #{report.shiftNumber} · {report.cashierName}
              </p>
            )}
          </div>
          {!closed && (
            ...
            <>
              ...
              <div className="space-y-1.5">
                ...
                {report.refundTotal > 0 && (
                  <Row label="(−) Devoluciones" value={formatCLP(report.refundTotal)} muted />
                )}
              ...
              {printStatus !== 'idle' && (
                <div
                  className="flex items-center gap-2 px-3 py-2 rounded text-sm font-body"
                  style={{
                ...
                >
                  ...
                  <span>
                    {printStatus === 'printing' && 'Imprimiendo Z-Report…'}
                    {printStatus === 'success'  && '✓ Z-Report impreso'}
                    {printStatus === 'error'    && (printError || 'Error de impresora')}
                  </span>
                  {printStatus === 'error' && (
                    <button
                      onClick={handleReprint}
                      className="ml-auto font-body text-xs underline"
                    >
                      Reintentar
                    </button>
                  )}
        ...
        >
          {!closed ? (
            <>
              ...
              >
                {closing ? 'Cerrando turno…' : 'Cerrar turno e imprimir'}
              </button>
            </>
          ) : (
            <>
...
function Row({ label, value, bold, muted }: { label: string; value: string; bold?: boolean; muted?: boolean }) {
  return (
    <div className="flex justify-between items-baseline">
      ...
      <span
        className={`font-mono text-sm ${bold ? 'font-black text-base' : 'font-medium'}`}
