...
import type { TillZReport } from '@/lib/types'

interface TillZReportModalProps {
  tillSessionId: string
  apiUrl:        string
  onClose:       () => void
  onClosed:      () => void  // till successfully closed — caller decides next step
}

type PrintStatus = 'idle' | 'printing' | 'success' | 'error'

export function TillZReportModal({ tillSessionId, apiUrl, onClose, onClosed }: TillZReportModalProps) {
  const [report,      setReport]      = useState<TillZReport | null>(null)
  ...
  useEffect(() => {
    fetch(`${apiUrl}/api/till-sessions/${tillSessionId}/z-report`, { credentials: 'include' })
      .then(r => r.json() as Promise<{ zReport: TillZReport }>)
      .then(d => setReport(d.zReport))
      .catch(() => setError('No se pudo cargar el reporte'))
      .finally(() => setLoading(false))
  }, [apiUrl, tillSessionId])
  ...
  const handleClose = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`${apiUrl}/api/till-sessions/${tillSessionId}/close`, {
        method:      'POST',
        credentials: 'include',
      })
      const data = await res.json() as { zReport: TillZReport; error?: string }
      ...
      setReport(data.zReport)
      setClosed(true)

      setPrintStatus('printing')
      const result = await printZReport(data.zReport)
  ...
  }, [apiUrl, tillSessionId, closing, closed, onClosed])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)' }}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b shrink-0" style={{ borderColor: 'var(--color-border)' }}>
          <div>
            <p className="font-headline font-bold text-lg" style={{ color: 'var(--color-text)' }}>Cierre de Caja</p>
            {report && (
              <p className="font-mono text-xs" style={{ color: 'var(--color-text-muted)' }}>
                Turno #{report.shiftNumber} · Caja #{report.tillSessionNumber} · {report.cashierName}
              </p>
            )}
          </div>
          {!closed && (
          ...
          {report && (
            <>
              <div className="space-y-1.5">
                <Row label="Tickets emitidos" value={String(report.ticketCount)} />
                {report.voidCount > 0    && <Row label="Anulaciones"   value={String(report.voidCount)} muted />}
                {report.refundCount > 0  && <Row label="Devoluciones"  value={String(report.refundCount)} muted />}
              </div>
              <Divider />
              <div className="space-y-1.5">
                ...
                <Row label="VENTA NETA" value={formatCLP(report.netTotal)} bold />
              </div>
              <Divider />
              <div>
              ...
              <div className="space-y-1.5">
                <p className="font-body text-[10px] font-semibold tracking-widest mb-2.5"
                   style={{ color: 'var(--color-text-muted)', letterSpacing: '0.14em' }}>CUADRE CAJA EFECTIVO</p>
                <Row label="Fondo inicial"       value={formatCLP(report.openingFloat)} />
                <Row label="(+) Efectivo ventas" value={formatCLP(report.byMethod.cash ?? 0)} />
                <Row label="EFECTIVO ESPERADO"   value={formatCLP(report.expectedCash)} bold />
              </div>
              ...
              {printStatus !== 'idle' && (
                <div className="flex items-center gap-2 px-3 py-2 rounded text-sm font-body"
                  ...
                  <span>
                    {printStatus === 'printing' && 'Imprimiendo Z-Caja…'}
                    {printStatus === 'success'  && '✓ Z-Caja impreso'}
          ...
          {!closed ? (
            <>
              ...
              <button
                ...
                {closing ? 'Cerrando caja…' : 'Cerrar caja e imprimir'}
              </button>
            </>
          ) : (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded font-body text-sm transition-all w-full justify-center"
              style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', color: 'var(--color-text)' }}>
